...
export default function RecruitmentDetailScreen() {
  ...
  const recruitment = useRecruitments().find((r) => r.id === id);
  ...
  const team = teams.find((t) => t.name === recruitment.teamName);
  const application = applications.find((a) => a.recruitmentId === recruitment.id);
  const remaining = remainingSlots(recruitment);
  const isClosed = recruitment.closed || remaining === 0;
  const isFavorite = favorites.has(recruitment.id);

  const onApply = () => {
    const a = apply(recruitment, 'はじめまして!募集を見て応募しました。よろしくお願いします!');
    ...
    <SafeAreaView style={styles.safeArea} edges={[]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.photoWrap}>
          <Image source={{ uri: recruitment.photo }} style={styles.photo} contentFit="cover" />
          <SafeAreaView edges={['top']} style={styles.photoTop}>
            <Pressable onPress={() => router.back()} style={styles.backButton} hitSlop={8}>
              <Ionicons name="chevron-back" size={22} color="#ffffff" />
            </Pressable>
            <Pressable
              onPress={() => toggleFavorite(recruitment.id)}
          ...
          <View
            style={[
              styles.typeBadge,
              { backgroundColor: RecruitmentTypeColors[recruitment.type] },
            ]}>
            <SportIcon sport={recruitment.sport} size={12} color="#ffffff" />
        ...
        <View style={styles.body}>
          ...
          <View style={styles.badgeRow}>
            <View
              style={[styles.levelBadge, { borderColor: LevelColors[recruitment.level] }]}>
              {recruitment.level === 'serious' && (
                <FlameIcon size={11} color={LevelColors[recruitment.level]} />
              )}
              <Text
                style={[styles.levelText, { color: LevelColors[recruitment.level] }]}>
                {LevelLabels[recruitment.level]}
              </Text>
            </View>
            {isClosed ? (
          ...
          <View style={styles.infoCard}>
            <InfoRow
              ...
              label="日時"
              value={`${formatDateWithWeekday(recruitment.date)} ${recruitment.startTime}〜${recruitment.endTime}`}
            />
            <InfoRow
              icon="location-outline"
              label="場所"
              value={`${recruitment.venueName}(${recruitment.ward})`}
            />
            <InfoRow
              icon="cash-outline"
              label="参加費"
              value={recruitment.fee === 0 ? '無料' : `¥${recruitment.fee.toLocaleString()}`}
            />
            <InfoRow
              icon="pricetag-outline"
              label="競技"
              value={SportLabels[recruitment.sport]}
            />
            <InfoRow
              icon="people-outline"
              label="募集人数"
              value={`${recruitment.capacity}人(${recruitment.filled}人参加済み)`}
            />
          </View>
