...
import { LevelColors, Palette, RecruitmentTypeColors, Spacing } from '@/constants/theme';
...
import type { Team, TeamMatch } from '@/types/team';
...
export default function TeamSiteScreen() {
  ...
  const allRecruitments = useRecruitments();
    ...
    <SafeAreaView style={styles.safeArea} edges={[]}>
      ...
      <ScrollView
        ...
        <View style={styles.innerWide}>
          ...
          {tab === 'recruit' && <RecruitPage team={team} recruitments={recruitments} />}
...
/** 募集ページ */
function RecruitPage({ team, recruitments }: { team: Team; recruitments: Recruitment[] }) {
  const styles = useThemedStyles(makeStyles);
  return (
    <View style={styles.body}>
      ...
      {recruitments.length === 0 && !team.memberRecruitNote ? (
        <Text style={styles.emptyText}>現在募集はありません</Text>
      ) : (
        recruitments.map((r) => <RecruitmentRow key={r.id} item={r} />)
      )}
...
/** 募集枠1行(タップで募集詳細へ) */
function RecruitmentRow({ item }: { item: Recruitment }) {
  const router = useRouter();
  const styles = useThemedStyles(makeStyles);
  ...
  return (
    <Pressable
      ...
      onPress={() =>
        router.push({ pathname: '/recruitment/[id]', params: { id: item.id } })
      }>
      <View
        style={[
          styles.recruitmentType,
          { backgroundColor: RecruitmentTypeColors[item.type] },
          isClosed && styles.recruitmentTypeClosed,
        ]}>
        <Text style={styles.recruitmentTypeText}>
          {isClosed ? '締切' : RecruitmentTypeLabels[item.type]}
        </Text>
      </View>
      <View style={styles.recruitmentBody}>
        <Text style={styles.recruitmentTitle} numberOfLines={1}>
          {item.title}
        </Text>
